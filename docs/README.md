...
**Phases 0 through 4 are complete — the product works.**
...
- `/` starts or joins a session. `/room/[code]` is the two-person booth: share a code
  ...
  instant**. You see each other side by side while posing — a live rehearsal of the
  card slot — and each device composites the same card locally, so you both download
  your own copy.
...
**Next up: Phase 5** — polish. Filters, frame colours, shareable card pages.
